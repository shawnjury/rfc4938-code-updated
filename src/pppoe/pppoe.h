...
/* rfc4938 and credit/metric scaling */
    ...
    UINT16_t local_credits_scalar; /* scalar for these credits */
    UINT16_t peer_credits;         /* peer credits per rfc4938 */
    UINT16_t peer_credits_scalar;  /* scalar for these credits */
    UINT16_t grant_amount;         /* amount we are going to grant in PADG */
    UINT16_t my_port;              /* port to listen to pkts on */
    UINT16_t peer_port;            /* port to send pkts to */
    UINT16_t parent_port;          /* port for parent rfc4938 proc */
    struct sockaddr_in my_sock;    /* my sock_addr */
    struct sockaddr_in peer_sock;  /* peer sock_addr */
    char *peer_ip;                 /* peer ip address string */
    int socket;                    /* socket to use */
    UINT8_t send_inband_grant;     /* flag to set to send an inband grant */
    UINT8_t padg_retries;          /* number of times a padg has been sent */
