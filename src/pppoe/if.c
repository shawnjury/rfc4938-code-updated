...
etherType(PPPoEPacket *packet)
{
    ...
    if (type != Eth_PPPOE_Discovery && type != Eth_PPPOE_Session) {
        PPPOE_DEBUG_ERROR("pppoe(): Invalid ether type 0x%x", type);
    }
    return type;
}
...
openInterface(char const *ifname, UINT16_t type, unsigned char *hwaddr)
{
    ...
    PPPOE_DEBUG_EVENT("pppoe(): Interface=%.16s HWaddr=%02X:%02X:%02X:%02X:%02X:"
                      "%02X Device=%.32s Buffer size=%d",
                      ifname,
...
openInterface(char const *ifname, UINT16_t type, unsigned char *hwaddr)
{
    ...
    if (hwaddr) {
        // sjury strncpy(ifr.ifr_name, ifname, sizeof(ifr.ifr_name));
...
#ifdef DL_DEBUG
        printf("pppoe(): %02x:%02x:%02x:%02x:%02x:%02x %02x:%02x\n",
...
receivePacket(int sock, PPPoEPacket *pkt, int *size)
{
    ...
    if (bpfSize < sizeof(hdr)) {
        PPPOE_DEBUG_ERROR("pppoe(): Truncated bpf packet header: len=%d", bpfSize);
        clearPacketHeader(pkt);                /* resets bpfSize and bpfOffset */
        return 0;
    }
    memcpy(&hdr, bpfBuffer + bpfOffset, sizeof(hdr));
    if (hdr.bh_caplen != hdr.bh_datalen) {
        PPPOE_DEBUG_ERROR("pppoe(): Truncated bpf packet: caplen=%d, datalen=%d",
    ...
    if (seglen > bpfSize) {
        PPPOE_DEBUG_ERROR("pppoe(): Truncated bpf packet: seglen=%d, bpfSize=%d",
               seglen, bpfSize);
        clearPacketHeader(pkt);                /* resets bpfSize and bpfOffset */
        return 0;
    }
